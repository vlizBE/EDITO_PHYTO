...
*This pdf file is also available as a Rmarkdown (Folder: Workspace > VRE Folders > Zoo-Phytoplankton_EOV > Modelling_phyto_and_zooplankton_interactions > Manual NPZ model > Manual_NPZD_model.Rmd). Store this file in your workspace and open it in Rstudio to have a interactive document.*
...
# Content Table
  ...
  2. Preparation
    ...
    2.5 Rpackages
  3. NPZ Model
    3.1 Calibration: Run the NPZ model
    3.2 Calibration: Calculate the Root Mean Square Error
    3.3 Simulation: Run the NPZ model a second time
...
We start from describing the input data, Rscripts and functions that were used for this Nutrient-Phytoplankton-Zooplankton (NPZ) model. Then, this manual will guide you step by step on how to run the NPZ model. At the end, phyto- and zooplankton dynamics will be simulated and the relative contributions of the environmental parameters estimated.
...
## 2.2 Working Folder
Let's create some working folders first. This is done to structure the work and make sure that the model can find all data it needs. By following this structure, less adjustments to directories are needed. We will create folders in our workspace to store our script, data and output. Input data and Rscripts can be copied from the 'Manual NPZ folder' to our working folder.


```{r,echo=FALSE}
# create a working folder to store your input data, rscripts and results
dir.create("~/work/NPZD") # create a working folder for your results
wd <- "~/work/NPZD/"

# copy and past files using R
#current_folder_input_data <- "~/workspace/VREFolders/Zoo-Phytoplankton_EOV/Modelling_phyto_and_zooplankton_interactions/Manual NPZ model/Input data"
#current_folder_Rscripts <- "~/workspace/VREFolders/Zoo-Phytoplankton_EOV/Modelling_phyto_and_zooplankton_interactions/Manual NPZ model/Rscripts"
#new_folder <- "~/work/NPZD"
...
dir.create("~/work/NPZD/Output") # create an output folder for your results
dir.create("~/work/NPZD/Output/Intermediate results") # create an subfolder for your intermediate results
dir.create("~/work/NPZD/Output/Final results") # create an subfolderfolder for your final results
dir.create("~/work/NPZD/Output/Final results/NPZ") # create an subfolderfolder for your final NPZ results
dir.create("~/work/NPZD/Output/Final results/Relative contributions") # create an subfolderfolder for your relative contributions results
```
Now, there should be a folder called 'NPZ' in your workspace, containing the subfolder 'Input data','Rscripts' and 'Output'. These subfolder contain subfolders and data or Rscripts. Having these files in our workspace, makes it possible to run the NPZ model.


## 2.3 Input Data
The input data that is required to perform a NPZ model is:
  1. Sea Surface Temperature (SST) in °C
  2. Nutrients (DIN, PO~4~, SiO~4~) in mmol N m<-3> (or P or Si equivalent)
...
Further, before running the NPZ model, the input data should be in the right format, i.e. .csv files, same order of the columns and same column names (Table 1), and the input data should be in the same units (see above). 

*Table 1*. An example of how the the input data format looks like.
```{r, echo=FALSE, results='asis'}
library(knitr)
InputData_example <- read.csv("~/work/NPZD/Input data/inputData_npzd_stationnearshore.csv", header = TRUE)
kable(head(InputData_example), align = "c")
```


## 2.4 Rscripts
The Rscripts in the subfolder 'NPZ' under Rscript contain functions that are used to run the model. They are called and loaded automatically when running this Rmarkdown.
...
The Rscripts should be stored in the Rscripts folder ("~/work/NPZD/Rscripts") that we created in the previous section.


## 2.5 Rpackages
Before we start running the NPZ model, we want to make sure all the needed Rpackages are properly installed. With the code below, we check if the Rpackages that we will use are installed. If this is not the case, it will install the Rpackages that are missing. 
```{r, echo=FALSE}
source("~/work/NPZD/Rscripts/NPZD/function_installed_packages.R")
...
## 3.1 Calibration: Run the NPZ model
We can start calibrating the NPZ model. First, we run the NPZ with threshold values aggregated from literature for each parameter.
...
```{r, echo=FALSE}
#NPZ Model - Parallel version
#Iteration 1

##########
source("~/work/NPZD/Rscripts/NPZD/functions_NPZD_parallel_iter1.R")
library(parallel)
library(foreach)
library(doParallel)
```
...
text_NPZD <- foreach(i = 1:numSimulations, .combine = rbind) %dopar% {
    tryCatch({
      npzd_run(as.numeric(stopdate - startdate)+1, possible_parameters[i,], station_region, inputData, station_lat, station_lon)   
...
```{r, echo=FALSE}
#Error analysis NPZ model
source("~/work/NPZD/Rscripts/NPZD/functions_error_v1.R")
```
...
```{r, echo=FALSE, results='asis'}
library(knitr)
InputData_example <- read.csv("~/work/NPZD/Input data/abiotic_pigment_2020.csv", header = TRUE)
kable(head(InputData_example), align = "c")
```
...
observations <- get_abiotic_pigment(startdate, stopdate, station_code,station_region) #read.csv(paste0(wd, "Input data/NPZD/abiotic_pigment_2020.csv"))
...
chla<-subset(subset(chla, Time > startdate_obs_cal), Time < stopdate_obs_cal)
...
```{r, echo=FALSE, results='asis'}
library(knitr)
InputData_example <- read.csv("~/work/NPZD/Input data/zooscan.csv", header = TRUE)
kable(head(InputData_example), align = "c")
```
...
zoo_observations<-subset(subset(zoo_observations, Date > startdate_obs_cal), Date < stopdate_obs_cal)
...
```{r, echo=FALSE}
simulations_error <- calculateErrors(station_region, chla, zoo_aggregated, numSimulations)
...
#based on RMSE
bestSim_graphs_rmse <- createBestSim(min_error_rmse$ID_sim, station_region, chla, zoo_aggregated)
...
bestSim_graphs_spring <- createBestSim(min_error_rmse2$ID_sim, station_region, chla, zoo_aggregated)
...
bestSim_graphs_summer <- createBestSim(min_error_rmse3$ID_sim, station_region, chla, zoo_aggregated)
...
## 3.3 Simulation: Run the NPZ model
Let's run the NPZ model a second time, but now we use the updated threshold values created in the previous iteration. 
...
##########
source("~/work/NPZD/Rscripts/NPZD/functions_NPZD_parallel_iter2_artifact.R")
library(parallel)
library(foreach)
library(doParallel)
```
Similar to the first step (3.1), we provide the station of interest and load the input data. Further, we decide how many iterations the NPZ model will do. For our publication, we selected 5000 as a good balance between quality and processing time. Then, we created 5000 starting combinations for the parameters based on their threshold value defined by the previous iteration. This is done in twofold, once for the spring condition and once for the summer-autumn conditions. In addition, we write .csv files for both conditions, with the possible parameters and store it in the output folder. 
```{r, echo=FALSE}
###########################
#Get data generated using GAM for DIN, PO4, SiO4, and temperature
#get data from station of interest

#station_region <-"nearshore"
#station_lat <- 51.256# coordinates station: latitude 
#station_lon <- 2.873 # coordinates station: longitude
#station_region <- "midshore"
#station_region <-"offshore"
inputData <- read.csv(paste0(wd, "Input data/inputData_npzd_station", station_region,".csv")) # or use get_inputdata(station_region, station_code, startdate, stopdate)
...
text_NPZD <- foreach(i = 1:numSimulations, .combine = rbind, .packages = "lubridate") %dopar% {
  tryCatch({
    npzd_run(as.numeric(stopdate - startdate)+1, possible_parameters_1[i,], possible_parameters_2[i,], station_region, inputData, station_lat, station_lon)   
  }, error = function(e) return(paste0("Error in NPZD iteration: ", i, 
                                       ", caused error: ", e)))        
...
```{r, echo=FALSE}
#Error analysis NPZ model
source("~/work/NPZD/Rscripts/NPZD/functions_error_iter2.R")
...
#Get observations of Chla and zooplankton
observations <- read.csv(paste0(wd, "Input data/abiotic_pigment_2020.csv")) #or use get_abiotic_pigment(startdate, stopdate, station_code,station_region)
observations$Station[observations$Station == station_code] <- station_region
#levels(observations$Station)[levels(observations$Station) == station_code] <- "midshore"
#levels(observations$Station)[levels(observations$Station) == station_code] <- "offshore"
...
chla$Time<-as.Date(chla$Time)
chla <- subset(subset(chla, Time > startdate_obs_val), Time < stopdate_obs_val)
...
```{r, echo=FALSE, include=FALSE}
#Zooplankton data
zoo_observations <- read.csv(paste0(wd, "Input data/zoo_converted_station", station_region, ".csv"))

#remove outliers only for station nearshore
zoo_observations <- zoo_observations[which(zoo_observations$n_mol < 0.5),]

simulations_error <- calculateErrors(station_region, chla, zoo_observations, numSimulations)
```
...
#####Create graphs of results with best simulations as time series
#based on RMSE
bestSim_graphs_rmse <- createBestSim(min_error_rmse$ID_sim, station_region, chla, zoo_observations)
```
...
source("~/work/NPZD/Rscripts/NPZD/functions_relativeContributions_iter2.R")
library(viridis)
```
...
```{r, echo=FALSE}
source("~/work/NPZD/Rscripts/NPZD/functions_graphsManuscript_v1.R")
source("~/work/NPZD/Rscripts/NPZD/functions_relativeContributions_iter2.R")
library(viridis)
...
bestSim_station <- getBestSim(station_region)
...
```{r, echo=FALSE}
dir.create("~/work/NPZD/Output/Graphs") # create an subfolder for your graphs
ggsave("Phyto- and zooplankton dynamics.jpeg", plot = comparisonGraphAvg_station_chla_zoo, path = "~/work/NPZD/Output/Graphs",
       width = 28,  height = 14,  units = "cm", dpi = 300)
ggsave("Phytoplankton dynamics.jpeg", plot = comparisonGraphAvg_station_chla, path = "~/work/NPZD/Output/Graphs",
       width = 28,  height = 14,  units = "cm", dpi = 300)
ggsave("Zooplankton dynamics.jpeg", plot = comparisonGraphAvg_station_zoo, path = "~/work/NPZD/Output/Graphs",
       width = 28,  height = 14,  units = "cm", dpi = 300)
ggsave("Relative contributions.jpeg", plot = comparisonGraph, path = "~/work/NPZD/Output/Graphs",
       width = 28,  height = 14,  units = "cm", dpi = 300)
ggsave("Boxplot relative contributions.jpeg", plot = comparisonContribution, path = "~/work/NPZD/Output/Graphs",
